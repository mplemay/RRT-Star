...
/**
 * Returns a normal distribution of the values based on the maxima && minima.
 *
 * @param waypoints Additional values to adjust the maxima && minima with.
...
CostMap2D::sample_space(const std::vector<Point2D> &waypoints) const {
  ...
  std::for_each(waypoints.begin(), waypoints.end(),
                [&s_min_x, &s_max_x, &s_min_y, &s_max_y](const auto &point) {
  ...
  return std::make_tuple(std::uniform_int_distribution<>(s_min_x, s_max_x),
                         std::uniform_int_distribution<>(s_min_y, s_max_y));
}
